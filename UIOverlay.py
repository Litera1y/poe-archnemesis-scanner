...
class UIOverlay:
    ...
    def _hotkey_pressed(self) -> None:
        if self._scan_label_text.get() == '스캔':
            self._scan(None)
        elif self._scan_label_text.get() == '숨기기':
            self._hide(None)
    ...
    def _scan(self, _) -> None:
        ...
        if len(results) > 0:
            ...
            if shopping_list_mode:
                ...
                trash_recipe = ('쓰레기', trash_recipe_items, False, True)
    ...
    def _show_recipe_browser_tree(self, item: str, results: Dict[str, List[Tuple[int, int]]]) -> None:
        ...
        if len(parents) > 0:
            tk.Label(self._recipe_browser_window, text='상위조합:', bg=COLOR_BG, fg=COLOR_FG_GREEN, font=FONT_BIG).grid(row=0, column=0)
...
class Settings:
    ...
    def show(self) -> None:
        ...
        tk.Button(self._window, text='스캔/숨기기 단축키', command=self._update_scan_hotkey).grid(row=3, column=1)
        ...
        c = tk.Checkbutton(self._window, text='모든 조합 표시', command=self._update_display_unavailable_recipes)
        c.grid(row=5, column=0, columnspan=2)
        if self._display_unavailable_recipes:
            c.select()

        c = tk.Checkbutton(self._window, text='조합식 클립보드 복사', command=self._update_copy_recipe_to_clipboard)
        c.grid(row=6, column=0, columnspan=2)
        if self._copy_recipe_to_clipboard:
            c.select()

        c = tk.Checkbutton(self._window, text='오버레이로 동작', command=self._update_run_as_overlay)
        c.grid(row=7, column=0, columnspan=2)
        if self._run_as_overlay:
            c.select()

        c = tk.Checkbutton(self._window, text='즐겨찾기 사용', command=self._update_shopping_list_mode)
        c.grid(row=8, column=0, columnspan=2)
        if self._shopping_list_mode:
            c.select()

        self._shopping_list_label = tk.StringVar()
        self._shopping_list_label.set("강적명을 쉼표로 구분")
        ...
        tk.Button(self._window, text='즐겨찾기 목록 저장', command=self._update_shopping_list).grid(row=10, column=1)
    ...
    def _update_shopping_list(self) -> None:
        ...
        for item in shopping_list:
            if item not in self._items_map.items():
                ...
                return
        self._update_shopping_list_label("즐겨찾기 목록 갱신!")
